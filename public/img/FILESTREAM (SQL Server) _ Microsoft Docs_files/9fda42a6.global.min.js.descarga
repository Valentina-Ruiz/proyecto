(function() {
    ...
    function readGroupsFromContent(content, config, selectionOptions) {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (isPreCode) {
                ...
                if (interactiveType !== undefined && interactiveType.name === "http" && msDocs.data.pageTemplate === "Conceptual") {
    ...
    function setupLocaleLink(userLocale) {
        ...
        if (location.hostname.indexOf("techprofile.microsoft.com") === -1) {
            ...
            localeSelector.href = "https://docs.microsoft.com/" + userLocale + "/locale?target=" + location.href;
        }
    }
    var platformConfig = {
        dotnet: {
    ...
    var Quiz = function() {
        function Quiz(form, bus) {
            ...
            bus.subscribe(QuizValidationResponseEvent, this.handleValidationEvent.bind(this));
        ...
        Quiz.prototype.handleValidationEvent = function(event) {
            ...
            if (!event.passed) {
                ...
                questionFeedbackElement.innerHTML = "Nailed it!";
            }
        };
        return Quiz;
    }();
    ...
    var QuizValidationResponseEvent = function() {
        function QuizValidationResponseEvent(response) {
            this.updated = response.updated;
            this.passed = response.passed;
            this.details = response.details;
        }
        return QuizValidationResponseEvent;
    }();
    ...
    function putUnitProgress(unitId, payload) {
        var query = {
            branch: branch,
            locale: locale
        };
    ...
    function createRequest$1(url, init) {
        init = init || {};
        init.mode = "cors";
    ...
    function moduleUnitPage() {
        ...
        validationService = new RemoteValidationService();
        ...
        currentModule.then(function(module) {
            ...
            renderModuleNavigation(unitIndex, module).then(function(navItems) {
                navigationItems = navItems || [];
                eventBus.subscribe(ProgressUpdatedEvent, function(event) {
                    updateNavigationItems(navigationItems, event.moduleProgress);
                });
                progressService.getProgress().then(function(data) {
                    eventBus.publish(new ProgressUpdatedEvent(data));
                });
                switch (completionType) {
                  ...
                  case "quiz":
                    setupQuizCompletionType(unitId);
                    break;
    ...
    function setupQuizCompletionType(unitId) {
        var bus = eventBus;
        ...
        bus.subscribe(QuizValidationRequestEvent, function(_a) {
            ...
            validationService.validateQuiz(unitId, answers).then(function(response) {
                bus.publish(new QuizValidationResponseEvent(response));
                if (response.passed) {
    ...
    function renderModuleNavigation(unitIndex, module) {
